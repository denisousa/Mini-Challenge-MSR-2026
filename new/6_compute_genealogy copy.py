...
# 5. Update Stats (By Project)
stats_updates_project = defaultdict(lambda: defaultdict(lambda: {
    "change": defaultdict(int),
    "evolution": defaultdict(int)
}))

# 6. Creation Stats (By Project)
stats_creation_project = defaultdict(lambda: defaultdict(int))
...
for filename in os.listdir(results_03_path):
    ...
    # 3. Extract Project Name (parts[2] and parts[3] joined with '_')
    try:
        project_name = f"{parts[2]}_{parts[3]}"
    except IndexError:
    ...
    for version in root.findall(".//version"):
        ...
        if is_start_of_lineage(change):
            ...
            stats_creation_project[project_name][author_group] += 1
        else:
            ...
            # D. Project Stats
            stats_updates_project[project_name][author_group]["change"][change] += 1
            if not is_start_of_lineage(evolution):
                stats_updates_project[project_name][author_group]["evolution"][evolution] += 1
...
df_agent["is_dev"] = df_agent["Author PR"] == "human"
...
df_agent_ns["is_dev"] = df_agent_ns["Author PR"] == "human"
...
# --- 7. PROJECT CREATION STATS ---
rows_creation_proj = []
for project, groups in stats_creation_project.items():
    for group, count in groups.items():
        rows_creation_proj.append({"Project": project, "Group": group, "New Clones Created": count})
df_creation_proj = pd.DataFrame(rows_creation_proj)
df_creation_proj = df_creation_proj.sort_values(by=["Project", "Group"])
df_creation_proj.to_csv(os.path.join(results_04_path, "7_clones_created_by_project.csv"), index=False)
print("Table 7 (Project Creation) Saved.")

# --- 8. PROJECT PATTERNS (Standard - Single Total) ---
rows_project = []
for project, groups in stats_updates_project.items():
    for group, types in groups.items():
        rows_project.append(get_pct_row({"Project": project, "Group": group}, types))
df_project = pd.DataFrame(rows_project, columns=["Project", "Group"] + columns_pct)
df_project = df_project.sort_values(by=["Project", "Group"])
df_project.to_csv(os.path.join(results_04_path, "8_project_patterns_dev_vs_agent.csv"), index=False)
print("Table 8 (Project Patterns) Saved.")

# --- 9. PROJECT PATTERNS (No Same) ---
rows_project_ns = []
for project, groups in stats_updates_project.items():
    for group, types in groups.items():
        rows_project_ns.append(get_pct_row_no_same({"Project": project, "Group": group}, types))
df_project_ns = pd.DataFrame(rows_project_ns, columns=["Project", "Group"] + columns_pct_no_same)
df_project_ns = df_project_ns.sort_values(by=["Project", "Group"])
df_project_ns.to_csv(os.path.join(results_04_path, "9_project_patterns_dev_vs_agent_NO_SAME.csv"), index=False)
print("Table 9 (Project Patterns No Same) Saved.")
